...
CREATE TABLE RankMPA (
  RankMPA_id int PRIMARY KEY NOT NULL,
...
COMMENT ON COLUMN RankMPA.description IS 'Описание рейтинга';
...
COMMENT ON COLUMN Film.descriprion IS 'Описание фильма';
...
COMMENT ON TABLE FilmLikers IS 'Фильмы, понравившиеся пользователям';

COMMENT ON COLUMN FilmLikers.film_id IS 'Код фильма';

COMMENT ON COLUMN FilmLikers.user_id IS 'Код пользователя';
...
ALTER TABLE FilmLikers ADD CONSTRAINT FilmLikers_film_id_Film_film_id FOREIGN KEY (film_id) REFERENCES Film (film_id) ON DELETE RESTRICT ON UPDATE CASCADE;

ALTER TABLE FilmLikers ADD CONSTRAINT FilmLikers_user_id_UserInfo_user_id FOREIGN KEY (user_id) REFERENCES UserInfo (user_id) ON DELETE RESTRICT ON UPDATE CASCADE;
